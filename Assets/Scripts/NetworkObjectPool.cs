...
public class NetworkObjectPool : Singleton<NetworkObjectPool>
{
    ...
    public void ReturnNetworkObject(NetworkObject networkObject, GameObject prefab)
    {
        ...
        // In this simple example pool we just disable objects while they are in the pool. But we could call afunction on  the object here for more flexibility.
        SetNetworkObjectActiveClientRpc(go);
        //go.transform.SetParent(transform);
        if (!pooledObjects.ContainsKey(prefab))
        {
            // Si no existe, inicializamos una nueva cola para este prefab
            pooledObjects[prefab] = new Queue<NetworkObject>();
        }
        // Ahora podemos encolar el objeto con confianza
        pooledObjects[prefab].Enqueue(networkObject);
    }

    [ClientRpc]
    public void SetNetworkObjectActiveClientRpc(GameObject go)
    {
